...
**Por qué:** Tras **CURSO-61**, el front debía dejar de mezclar enrollments con assignments. Sprint cerrado: rejilla admin usa `GET /api/assignments`; matrículas de alumnos en `EnrollmentHttpService.getByFilters()`; «Mis clases» mantiene `GET /api/teachers/:id/subjects` por decisión explícita; Bruno validado (deprecados responden aviso en backend).

| Clave | Tipo | Estado | SP | Resumen | Qué se hizo / por qué | Repo / doc |
| --- | --- | --- | --- | --- | --- | --- |
| [**CURSO-62**](https://franciscocobsan.atlassian.net/browse/CURSO-62) | Historia | Finalizada | 5 | Migración front a GET /api/assignments | Historia padre cerrada; tareas 63–68 finalizadas | Jira [**CURSO-62**](https://franciscocobsan.atlassian.net/browse/CURSO-62); `services-for-week-schedule/` |
| [**CURSO-63**](https://franciscocobsan.atlassian.net/browse/CURSO-63) | Tarea | Finalizada | 2 | `AssignmentHttpService` | `getAll()`, `getByTeacher(id, schoolYear)` | `teacher-assignment-http.service.ts` |
| [**CURSO-64**](https://franciscocobsan.atlassian.net/browse/CURSO-64) | Tarea | Finalizada | 1 | Limpiar `EnrollmentHttpService` | Solo `getByFilters()`; sin `getAll`/`getByTeacher` deprecados | `enrollment-http.service.ts` |
| [**CURSO-65**](https://franciscocobsan.atlassian.net/browse/CURSO-65) | Tarea | Finalizada | 3 | Migrar assignment data (rejilla admin) | `WeekScheduleAssignmentDataService` → `GET /api/assignments` + filtro por grupo; `ClasesService` sin cambio | `week-schedule-assignment-data.service.ts` |
| [**CURSO-66**](https://franciscocobsan.atlassian.net/browse/CURSO-66) | Tarea | Finalizada | 1 | QA clases-profesor | Flujo «Mis clases» validado sin cambio de ruta | `pages/profesor/clases-profesor/` |
| [**CURSO-67**](https://franciscocobsan.atlassian.net/browse/CURSO-67) | Tarea | Finalizada | 2 | Centralizar `by-filters` alumnos | Delegación en `EnrollmentHttpService`; `environment.apiUrl` | `carga-studentspor-grupo-asignatura.service.ts`, `create-student-task.service.ts` |
| [**CURSO-68**](https://franciscocobsan.atlassian.net/browse/CURSO-68) | Tarea | Finalizada | 1 | Validación Bruno | assignments y by-filters OK; deprecados con aviso backend | Bruno / Postman del equipo |
