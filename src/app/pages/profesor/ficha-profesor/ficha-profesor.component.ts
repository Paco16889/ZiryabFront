  ...
  cargarJustificacionesPendientes(): void {
    ...
    this.assistanceService.getAllAssistances().subscribe({
      ...
      error: (err) => {
        ...
        this.statusMessage.set({text: this.translate.instant('teacherPages.justifications.loadError'), type: 'error'});
        this.loading.set(false);
      }
    });
  }
  ...
  aceptarJustificacion(assistanceId: number): void {
    ...
    this.assistanceService.justifyAbsence(assistanceId).subscribe({
      ...
      error: (err) => {
        ...
        this.statusMessage.set({text: this.translate.instant('teacherPages.justifications.errorAccept'), type: 'error'});
        this.loading.set(false);
      }
    });
  }
  ...
  rechazarJustificacion(assistanceId: number): void {
    ...
    this.assistanceService.rejectJustification(assistanceId).subscribe({
      ...
      error: (err) => {
        ...
        this.statusMessage.set({text: this.translate.instant('teacherPages.justifications.errorReject'), type: 'error'});
        this.loading.set(false);
      }
    });
  }
